...
public class AgentService : IAgentService
{
        ...
        if (IsDepartmentScopedStaff)
        {
            if (!_currentUser.PrimaryDepartmentId.HasValue)
            {
                return ServiceResult<IReadOnlyList<AgentDto>>
                    .Forbidden("Department access is not configured for this user.");
            }
            ...
            if (departmentId.HasValue &&
                departmentId.Value != currentDepartmentId)
            {
                return ServiceResult<IReadOnlyList<AgentDto>>
                    .Forbidden("You cannot access agents outside your department.");
            }

            agents = agents.Where(
                a => a.DepartmentId == currentDepartmentId);
        }
        ...
        CancellationToken ct = default)
    {
        if (IsDepartmentScopedStaff)
        {
            ...
            var targetDepartmentId = await _unitOfWork
                .Repository<Agent>()
                .Query()
                .AsNoTracking()
                .Where(a =>
        ...
        CancellationToken ct = default)
    {
        ...
        if (IsDepartmentScopedStaff)
        {
            ...
            if (agent.DepartmentId != currentDepartmentId ||
                dto.DepartmentId != currentDepartmentId)
            {
                return ServiceResult<AgentDto>
                    .Forbidden("You cannot update agents outside your department.");
            }
        }
        ...
        if (agent.DepartmentId != dto.DepartmentId)
        {
            var hasOpenTickets = await _unitOfWork
                ...
                .AnyAsync(
                    t =>
                        t.AssignedToUserId == agent.UserId &&
                        !t.IsDeleted &&
                        !t.Status.IsTerminal,
                    ct);
            ...
            if (hasOpenTickets)
            {
                return ServiceResult<AgentDto>
                    .Conflict(
                        "Agent has open tickets and cannot be moved to another department.");
            }
        }
