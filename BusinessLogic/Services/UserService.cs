...
public class UserService : IUserService
{
        ...
        CancellationToken ct = default)
    {
            ...
            !existingAgent.IsDeleted)
        {
            ...
            var isChangingAgentDepartment =
                wantsAgent &&
                dto.DepartmentId.HasValue &&
                existingAgent.DepartmentId != dto.DepartmentId.Value;

            if (isRemovingAgent ||
                isChangingAgentDepartment)
            {
                var hasOpenTickets =
                ...
                if (hasOpenTickets)
                {
                    return ServiceResult<UserDto>
                        .Conflict(
                            isChangingAgentDepartment
                                ? "The agent has open tickets and cannot be moved to another department."
